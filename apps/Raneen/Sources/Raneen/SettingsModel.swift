...
final class SettingsModel: ObservableObject {
    ...
    ///
    /// The reasons live on different panes, and a person who has just read
    /// "always open because a wake word is armed and recording is on"
    /// should not have to go and find each one. The trigger is left alone:
    /// if it is speech or a wake word, the microphone *is* the trigger, and
    /// changing that is a different decision made on the row above. Armed
    /// wake words are forgotten rather than disabled — a wake word here is
    /// a path to a file that stays on disk, and there is no "armed but not
    /// listening" state for a detector that runs on every frame. Speaker
    /// identification is not touched: it is not a reason (see
    /// `MicrophonePolicy`).
