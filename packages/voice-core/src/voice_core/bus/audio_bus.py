...
class AudioBusReader:
    ...
    def rewind(self, frames: int) -> int:
        """Move the cursor *back* up to ``frames``, returning how far it moved.

        The ring buffer already holds history — at 80 ms per frame the
        default capacity of 500 is ~40 s of it — so a consumer that learns
        late it should have been recording can go back and get the audio.

        This is what makes VAD-triggered dictation possible. The voice
        activity tracker only reports "speech started" after
        ``speech_threshold`` consecutive speech frames, so by the time a
        trigger fires, the first ~240 ms of the utterance is already in the
        past. Without a rewind the user's first word gets clipped; with it,
        the recording starts before they began speaking.

        Bounded on both ends: never past frame 0, and never further back
        than the buffer still holds (older frames have been overwritten).

        Args:
            frames: How many frames to try to go back. Values <= 0 are a
                no-op.

        Returns:
            The number of frames actually rewound, which may be less than
            requested near the start of a stream.
        """
        if frames <= 0:
            return 0
        with self._bus._condition:
            oldest_available = max(0, self._bus._write_pos - self._bus._capacity)
            target = max(oldest_available, self._read_pos - frames)
            moved = self._read_pos - target
            self._read_pos = target
            return moved
