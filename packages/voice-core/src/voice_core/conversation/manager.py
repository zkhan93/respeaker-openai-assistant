...
* Indicator choreography on transitions (``listen`` / ``think`` / ``speak`` /
  ``off``).
...
Indicator / speaker / event-bus calls are made *outside* the lock to keep
...
if TYPE_CHECKING:
    ...
    from ..pipeline.speaker import SpeakerManager
    from ..ports.indicator import Indicator
...
class ConversationManager:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            indicator: Status indicator — an LED ring on the Pi, a
                menu-bar icon on a laptop, a no-op when headless. This
                manager only names states; the adapter decides how they
                look. Pattern transitions are issued from here.
    ...
    def detach(self) -> None:
        ...
        # promptly. We don't touch the indicator here — :meth:`detach`'s
    ...
    def end_conversation(self, *, reason: str = "explicit") -> None:
        ...
        if ended is not None:
            # Move the indicator to "off" outside the lock so the publish doesn't
            # contend with hardware writes.
            self._indicator.set_pattern("off")
            if in_flight is not None:
    ...
    def _on_hotword(self, event: HotwordEvent) -> None:
        ...
        self._indicator.set_pattern("listen")
    ...
    def _on_voice_stopped(self, event: VoiceActivityEvent) -> None:
        ...
        if fire:
            self._indicator.set_pattern("think")
    ...
    def _on_transcription_completed(self, event: TranscriptionCompletedEvent) -> None:
        ...
        self._indicator.set_pattern("speak")
        logger.info(
    ...
    def _run_reply(self, ctx: ReplyContext) -> None:
        ...
        1. ``end_conversation(reason="error")`` is called *before*
           ...
           sweep, and turns the indicator off.
