...
class VoiceDetectionService:
    ...
    Voice activity events are published automatically by AudioPipeline.
    ...
    def __init__(
        ...
        Args:
            audio_pipeline: Capture pipeline to read frames from.
            event_bus: Event bus for publishing events
            hotword_detector: Hotword detector instance, or None to disable hotword
                detection (VAD events from AudioPipeline still flow through EventBus).
        ...
        self.reader = audio_pipeline.create_reader()
    ...
    def start(self):
        ...
        Voice activity events are emitted automatically by AudioPipeline.
        ...
        if self.hotword_detector is None:
            logger.warning(
                ...
                "from AudioPipeline, but no hotword events will be published."
            )
        ...
        try:
            while self.running:
                ...
                # Frames are already PCM16 mono — the AudioSource contract
                # guarantees it, so no conversion step is needed here.
                scores = self.hotword_detector.get_scores(audio_data)
