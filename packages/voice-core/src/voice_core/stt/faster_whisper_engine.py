...
class FasterWhisperSTT:
    ...
    def __init__(
        ...
        Args:
            ...
            beam_size: Beam-search width. Defaults to ``1`` (greedy)
                because the Pi is the constrained target and beam search
                roughly doubles inference time there.

                On a laptop that tradeoff inverts — inference is already
                far under realtime, so the accuracy is worth buying. The
                desktop app sets 5; see ``voice_desktop.settings``.
        """
        kwargs: dict = {
            "device": device,
    ...
    def transcribe(
        ...
        ``prompt`` is passed as Whisper's ``initial_prompt``, which
        conditions the decoder without being transcribed itself. See the
        :class:`~voice_core.stt.engine.STTEngine` protocol for why that
        matters when audio is cut into short independent segments.
        """
        ...
        segments, info = self._model.transcribe(
            ...
            # Text context for the decoder. Not transcribed — it only
            # shifts the priors, which is what carries vocabulary, casing
            # and punctuation style across a segment boundary.
            ...
            # Whisper's *internal* conditioning across the 30 s windows of
            # one call. Our segments are far shorter than one window, so
            # this does nothing for us either way — cross-segment context
            # comes from ``initial_prompt`` above, which the caller
            # controls and can reset. Leaving this off also avoids
            # Whisper's known degenerate-repetition loop.
